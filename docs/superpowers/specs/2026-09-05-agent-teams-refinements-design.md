...
- **自定义节点**（Vue Flow `member` 节点类型）：上部节点标题+编号+状态点；中部成员色点+名称+配置档案/人格覆盖标签；底部任务预览两行+入站依赖数；左入右出 handle（大连接区）。选中清晰边框；monitor 态五色+interrupted 橙；成员缺失/档案不存在 → 顶部可点击错误标记（定位到属性项）。节点宽约 220px。
